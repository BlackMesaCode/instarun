...
namespace InstaRun
{
    public class MouseHook
    {
        ...
        private delegate int MouseHookDelegate(int nCode, IntPtr wParam, IntPtr lParam);
        public delegate void MouseEventHandlerExtended(object sender, MouseEventArgsExtended e);

        public event MouseEventHandlerExtended ButtonDown;
        public event MouseEventHandlerExtended ButtonUp;
        public event MouseEventHandlerExtended OnMouseInput;

        public event MouseEventHandler Wheeled;

        private readonly MouseHookDelegate _myMouseHookDelegate;
        ...
        public MouseHook()
        {
            ...
            _myMouseHookDelegate = MouseHookCallback;
            MyHook = SetWindowsHookEx((int)HookType.WH_MOUSE_LL, _myMouseHookDelegate, hInstance, 0);
        }
        ...
        private int MouseHookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0)
            {
                // lParam has to be manually marshalled, in order to correctly cast into a mouseHookStruc
                var hookStruct = (MouseHookStruct)Marshal.PtrToStructure(lParam, typeof(MouseHookStruct));

                var buttonDown = MouseButtons.None;
                var buttonUp = MouseButtons.None;
                var delta = 0;
                ...
                int clickCount;
                if ((int)wParam == (int)WM.LBUTTONDBLCLK || (int)wParam == (int)WM.RBUTTONDBLCLK)
                    clickCount = 2;
                else
                    clickCount = 1;

                MouseEventArgsExtended mouseEventArgsExtended = null;

                if (buttonDown != MouseButtons.None)
                {
                    if (ButtonDown != null)
                    {
                        mouseEventArgsExtended = new MouseEventArgsExtended(buttonDown, clickCount, hookStruct.pt.x, hookStruct.pt.y, 0);
                        ButtonDown(this, mouseEventArgsExtended);
                ...
                if (buttonUp != MouseButtons.None)
                {
                    if (ButtonUp != null)
                    {
                        mouseEventArgsExtended = new MouseEventArgsExtended(buttonUp, clickCount, hookStruct.pt.x, hookStruct.pt.y, 0);
                        ButtonUp(this, mouseEventArgsExtended);
                    }
                }
        ...
        int Low16(int value)
        {
            return unchecked((short)value);   // classic unchecked cast to uint
        }
        int High16(int value)
        {
        ...
        protected struct MouseHookStruct
        {
            public Point pt;
        ...
        private static extern IntPtr SetWindowsHookEx(int idHook, MouseHookDelegate callback, IntPtr hInstance,
                                                      uint threadId);
